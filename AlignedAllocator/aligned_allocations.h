...
/// helper
/// \details check whether the address is aligned to `alignment` boundary
bool isAligned(const volatile void *p, std::size_t alignment)
{
	return (reinterpret_cast<std::uintptr_t>(p) % alignment) == 0;
}
bool isAligned(std::uintptr_t pi, std::size_t alignment)
{
...
template<typename T>
T* alignForward(T* p, std::size_t alignment) noexcept
{
	if (alignment == 0)
		return p;
...
const std::size_t getForwardPaddingWithHeader(const std::size_t p, const std::size_t alignment, 
...
template<typename T>
T* alignPtr(const T *ptr, const std::size_t alignment)
{
...
void* _alignedMalloc(std::size_t bytes, std::size_t alignment)	// FIXME: doesn't work properly
{
	assert(false);
	void *pmem = nullptr;
	...
	void** palignedMem = reinterpret_cast<void**>(
		reinterpret_cast<std::size_t>(pmem) + bytesForAdjustment
		);
	palignedMem[-1] = pmem;
	return palignedMem;
}
...
void* alignedMalloc(std::size_t count, std::size_t alignment)
{
	void* p;
#if defined(_MSC_VER) || defined(_WIN32) || defined(_WIN64)
	p = _aligned_malloc(count, alignment);
	if (p == 0)
	{
		throw std::bad_alloc();
	}
