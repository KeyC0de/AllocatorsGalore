...
int main()
{
	...
	// Example usage of StackAllocator No2
	...
	std::cout << "sizeof(StackAllocator<GameObject,64>)=" << sizeof(StackAllocator<GameObject, 64>) << '\n';

	std::cout << "sizeof(StackAllocator)=" << sizeof(StackAllocator<GameObject>) << '\n';
	std::cout << "10 * sizeof(GameObject) =" << 10 * sizeof(GameObject) << '\n';

	std::cout << "\nVector" << '\n';

	StackAllocator<GameObject> fooalloc(7000);
	std::vector<GameObject, StackAllocator<GameObject>> vec(fooalloc);
	...
	StackAllocator<GameObject, sizeof(GameObject)> goAlloc{10500};
	...
	StackAllocator<astring> la5{ 8192 };
	...
	StackAllocator<char, 32> sa{2048};
	...
	StackAllocator<void, 16> la1{ 2048 };
	...
	std::cout << "\nMap with StackAllocator bare" << '\n';
	using smapstring = std::basic_string<char, std::char_traits<char>, StackAllocator<char, 16>>;
	using samap = std::map<smapstring, int, std::less<>, StackAllocator<std::pair<smapstring const, int>, 16>>;
	StackAllocator<smapstring, 16> la2{ 2048 };
	...
	StackAllocator<int, 256> la6(1024);
	...
	*pint = 1453;
	...
	{
		StackAllocator<int, 16> salo{ 1024 };
		StackAllocator<int, 16> salo2{ salo };
		//StackAllocator<int, 32> salo3{salo2}; // incompatible!
		StackAllocator<int, 16> salo4 = std::move(salo);
		StackAllocator<char, 16> saloc{ salo2 };
		//StackAllocator<char, 32> saloc2 = std::move(salo2);	// nope!
		//StackAllocator<char, 32> saloc3 = std::move(saloc);	// nope!
		StackAllocator<void, 16> saloc4 = std::move(saloc);
		StackAllocator<void, 16> saloc5{ saloc4 };
