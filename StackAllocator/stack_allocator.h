...
public:
	...
	void* allocate(std::size_t bytes)
	{
		...
		std::size_t currentAllocationStartAddress = alignForward((std::size_t) m_poffset + sizeof(Header), s_alignment);
...
/// \brief	The stack allocator is an extension of the Linear allocator, with the additional ability
///			that it can also deallocate.
///	\brief	Although the StackAllocator is copyable, the Arena is not, so effectively copies become moves. 
...
inline bool operator==(const StackAllocator<T, tm_alignment>&, const StackAllocator<T, tm_otherAlignment>&)
{
	return true;
}
...
inline bool operator==(const StackAllocator<T, tm_alignment>&, const OtherAllocator&)
{
	return false;
}
// equivalent statement:
